...
if __name__ == "__main__":
    ...
    training_data = datasets.CIFAR10(
            root=r"C:\Users\pjmcc\PycharmProjects\dropoutResearch\data\CIFAR10",
            train=True,
            download=True,
            transform=ToTensor()
    )
    ...
    for j in tqdm(range(num_iters)):
        curr_model = BaseNoDropConvModel()  # Create new instance to reset parameters (and probabilities)
        ...
        for i in tqdm(range(epochs)):
            ...
            starttime = time.time()
            avg_t_loss, probs_l = Testing.train_loop(train_dataloader, curr_model, optimizer, loss_fn, device, i, mode=None, record=True,
                                    return_probs=False)
            avg_v_loss, v_acc = Testing.test_loop(test_dataloader, curr_model, loss_fn, device, i, mode=None, record=True)
            end_time = time.time()
    ...
    data_frame.to_csv(r"C:\Users\pjmcc\PycharmProjects\dropoutResearch\results\CIFAR10\CIFAR10NoDrop10x40.csv", mode='w')
